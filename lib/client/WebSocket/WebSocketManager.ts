...
class WebSocketManager extends EventEmitter {
  ...
  async connect() {
    ...
    this.shardQueue = new Set(
      shards.map((id: number) => new WebSocketShard(this, id)),
    );
  ...
  private async reconnect(): Promise<boolean> {
    ...
    try {
      ...
      this.debug(
        `Couldn't connect or fetch information about the gateway. ${error}`,
      );
  ...
  async handlePacket(packet?: any, shard?: WebSocketShard): Promise<boolean> {
